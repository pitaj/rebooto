...
pub fn set_privileged(
    ...
    if dry_run {
        ...
        println!("[dry run] set EFI variable `{refind_previousboot}` to '{next}' (was '{prev}')");
    } else {
        system
...
pub fn set(
    ...
    command.arg(&executable).arg(target_os).arg("--privileged");
...
pub use self::set_privileged as set;
